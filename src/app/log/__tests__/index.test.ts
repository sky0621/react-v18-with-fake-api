  ...
  describe('createConsoleDebugLog', () => {
    ...
    describe('one kind parameter', () => {
      test('only filePath parameter', () => {
        expect(createConsoleLog('pages/Home')([])).toEqual(
          '[ pages/Home ]\nPASS',
        );
      });
      ...
      test('only one string arg parameter', () => {
        expect(createConsoleLog()(['got response'])).toEqual('got response');
      });
      test('only one string array arg parameter', () => {
        expect(createConsoleLog()([['one', 'two']])).toEqual('["one","two"]');
      });
    ...
    describe('two kind parameters', () => {
      ...
      test('filePath and arg parameters', () => {
        expect(createConsoleLog('pages/Home')(['got response'])).toEqual(
          '[ pages/Home ]\ngot response',
        );
      });
      test('funcName and arg parameters', () => {
        expect(
          createConsoleLog(undefined, 'getUsers')(['got response']),
        ).toEqual('[ getUsers ]\ngot response');
      });
    });
